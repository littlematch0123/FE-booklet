...
&emsp;&emsp;下面来处理原数据表tdb_goods中的品牌信息，首先查询tdb_goods表的"品牌"，并分组
...
&emsp;&emsp;通过上面的操作，已经把重复的数据分散到不同的数据表中进行存储了，尽可能的节省存储空间了。但是，显示时，却需要把原来的数据显示出来，这就需要使用下面要介绍的概念&mdash;&mdash;连接
...
&emsp;&emsp;右外连接指显示右表的全部记录及左表符合连接条件的记录
