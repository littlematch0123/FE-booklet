...
　　和其他任何编程语言一样，javascript保留了一些[标识符](http://www.cnblogs.com/xiaohuochai/p/5549833.html#anchor1)为自己所用。这些保留字不能用做普通的标识符。由于好多参考书的误导，貌似保留字和关键字是分开的，其实并不是，关键字只是保留字的一部分。保留字包括关键字、未来保留字、空字面量和布尔值字面量
