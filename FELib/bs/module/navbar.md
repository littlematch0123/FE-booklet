...
　　导航条（navbar）和[导航](http://www.cnblogs.com/xiaohuochai/p/7111581.html)（nav），就相差一个字，多了一个&ldquo;条&rdquo;字。其实在Bootstrap框架中他们还是明显的区别。在导航条(navbar)中有一个背景色、而且导航条可以是纯链接（类似导航），也可以是表单，还有就是表单和导航一起结合等多种形式。本文将详细介绍Bootstrap导航条
...
　　1、首先在制作导航的列表(&lt;ul class=&rdquo;nav&rdquo;&gt;)基础上添加类名&ldquo;navbar-nav&rdquo;

　　2、在列表外部添加一个容器（div），并且使用类名&ldquo;navbar&rdquo;和&ldquo;navbar-default&rdquo;

【原理分析】

　　&ldquo;.navbar&rdquo;样式的主要功能就是设置左右padding和圆角等效果，但它和颜色相关的样式没有进行任何的设置
...
　　导航条的颜色都是通过&ldquo;.navbar-default&rdquo;来进行控制
...
　　navbar-nav样式是在导航.nav的基础上重新调整了菜单项的浮动与内外边距。同时也不包括颜色等样式设置，而颜色和其他样式是通过配合父容器&ldquo;navbar-default&rdquo;来一起实现

　　[注意]最好使用&nbsp;&lt;nav&gt;&nbsp;元素，如果使用的是通用的&nbsp;&lt;div&gt;&nbsp;元素的话，务必为导航条设置&nbsp;`role="navigation"`&nbsp;属性，这样能够让使用辅助设备的用户明确知道这是一个导航区域
...
　　在Web页面制作中，常常在菜单前面都会有一个标题（文字字号比其它文字稍大一些），其实在Bootstrap框架也做了这方面考虑，其通过&ldquo;navbar-header&rdquo;和&ldquo;navbar-brand&rdquo;来实现
...
　　有的导航条中会带有搜索表单，Bootstrap框架中提供了一个&ldquo;navbar-form&rdquo;，使用方法很简单，在navbar容器中放置一个带有navbar-form类名的表单即可
